...
CMD_ARGS="+set vid_ref glx +set gl_driver libGL.so.1 +set _windowed_mouse 1"
...
[ ! -d $HOME/.kingpin ] && mkdir $HOME/.kingpin
[ ! -r $HOME/.kingpin/kingpin.conf ] && cp ${GAMEDIR}/kingpin.conf $HOME/.kingpin


if [ "`head $HOME/.kingpin/kingpin.conf`" != "${GAMEDIR}" ]
then
	[ -w $HOME/.kingpin/kingpin.conf ] && echo ${GAMEDIR} > $HOME/.kingpin/kingpin.conf
fi

[ -e $HOME/.kingpin/main/save/current/game.ssv ] && rm $HOME/.kingpin/main/save/current/*

for directory in $HOME/.kingpin/main/save/save*
do
	[ -e $directory ] &&
		[ ! -e ${GAMEDIR}/main/save/`basename $directory` ] && 
			mkdir -p ${GAMEDIR}/main/save/`basename $directory`
done
...
export LD_PRELOAD="${GAMEDIR}/kphack.so:$LD_PRELOAD"
